...
const MovieDetails = ({ movie }: { movie: MovieDetail }) => {
  const { favMovies, removeFavoriteMovie, addFavoriteMovie } =
    useFavouriteMovies();
  ...
  const [videoTrailerId, setVideoTrailerId] = useState<string | undefined>(
    undefined,
  );
  ...
  return (
    <>
      <div
        style={{
          backgroundImage: `url(${baseCdnImage + '/uploads/movies/' + movie.poster_url})`,
        }}
