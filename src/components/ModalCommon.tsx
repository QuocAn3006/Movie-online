...
const ModalCommon: FC<ModalProps> = ({
  ...
  const [searchResult, setSearchResult] = useState<Movies | undefined>(
    undefined,
  );
            ...
            <div className="relative px-4 flex-auto">
              ...
              {loading && (
                <Spinner
                  ...
                  className="absolute top-4 right-7 w-4 h-4"
                />
              )}
            </div>
            {topMovies.length > 0 ? (
              <div className="w-full h-fit">
                <ul className="list-none w-full h-fit overflow-y-auto p-4">
                  {topMovies.map((item: Movie) => {
                    const moviePath = item.slug?.startsWith('/')
                      ? item.slug
                      : `/movies/${item.slug}`;
              ...
              <p className="w-full h-fit p-4">Không tìm thấy kết quả phù hợp</p>
            )}
          </div>
        </>
      )}
